...
with col1:
    ...
    if f1 is not None:
        try:
            ...
            if f1.type in ["csv", "txt"]:
                OCM = pd.read_csv(f1, encoding=encoding)
            elif f1.type == "xlsx":
                OCM = pd.read_excel(f1, engine='openpyxl')
            else:
                OCM = pd.read_excel(f1, engine='xlrd')
...
with col2:
    ...
    if f2 is not None:
        try:
            ...
            # Lecture du fichier en fonction de son type
            if f2.type in ["csv", "txt"]:
                MTN = pd.read_csv(f2, encoding=encoding)
            elif f2.type == "xlsx":
                MTN = pd.read_excel(f2, engine='openpyxl')
            else:
                MTN = pd.read_excel(f2, engine='xlrd')
            # Affichage d'un échantillon du dataframe
